...
dgdata_stmt.run([1, 10, faker.word.words(3).split(" ").join(", ")]);
dgdata_stmt.run([1, 11, faker.word.words(7).split(" ").join(", ")]);
...
const testuser_hash = await bcrypt.hash("123", 10);
const ro_user_hash = await bcrypt.hash("123", 10);
